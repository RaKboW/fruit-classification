...
class DatasetCatalog:
    ...
    def get(name):
        if "voc" in name:
            ...
            fru_root = DatasetCatalog.DATA_DIR
            if 'FRU_ROOT' in os.environ:
                fru_root = os.environ['FRU_ROOT']

            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                data_dir=os.path.join(fru_root, attrs["data_dir"]),
                split=attrs["split"],
            )
            return dict(factory="FRUDataset", args=args)
